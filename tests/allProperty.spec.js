...
test.describe("Booking.com All Properties Search", () => {
  /** @type {HomePage} */
  let homePage;
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await allure.feature('All Properties Sorting high rating and low price');
    await allure.story(`Find Top Property in ${searchConfig.searchData.destination} with high rating and low price`);
...
------------------------------------------------------------
  ...
  Detail page URL : ${winner.url}`;
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(' **  BEST PROPERTY (Selection Results) **');
    console.log('═'.repeat(60));
    console.log(resultLog);
    console.log('═'.repeat(60) + '\n');

    const exportedData = {
      testFeature: "All Properties Sorting high rating and low price",
    ...
    // Save locally
    const savedFilePath = exportWinnerData("AllProperty", testInfo.title, exportedData);
