...
test.describe("Booking.com Resort Search", () => {
    /** @type {HomePage} */
    let homePage;
    /** @type {HotelListPage} */
    let resultsPage;
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await allure.feature('Resort Filtering high rating and low price');
        await allure.story(`Find Top Resort in Destination (${searchConfig.searchData.destination}) with high rating and low price`);
    ...
    test('Booking.com Search and Scrape Top Result', async ({ page, browserName }, testInfo) => {
        ...
        await stepWithScreenshot("Apply the resort filter", page, async () => {
            await resultsPage.applyResortFilter();
            await page.waitForTimeout(2000);
        });
...
------------------------------------------------------------
  ...
  Detail page URL : ${winner.url}`;
        ...
        console.log('\n' + '═'.repeat(60));
        console.log('  **  BEST HOTEL / RESORT (Selection Results) **');
        console.log('═'.repeat(60));
        console.log(resultLog);
        console.log('═'.repeat(60) + '\n');

        const exportedData = {
            testFeature: "Resort Filtering high rating and low price",
        ...
        const savedFilePath = exportWinnerData("Resort", testInfo.title, exportedData);
